...
import { z } from 'zod';
import { Eye, EyeOff, UserPlus, Sparkles, ShieldCheck, User } from 'lucide-react';
...
export default function Register() {
  ...
  const onSubmit = async (data) => {
    try {
      await regUser({ name: data.name, email: data.email, password: data.password, role: data.role });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="auth-form">
          ...
          <div className="auth-field">
            ...
            <div className="auth-role-selector">
              <label className={`auth-role-option ${selectedRole === 'ADMIN' ? 'selected' : ''}`}>
                <input type="radio" value="ADMIN" {...register('role')} />
                <ShieldCheck size={18} />
                <div>
                  <span className="auth-role-title">Admin</span>
                  <span className="auth-role-desc">Full access & manage users</span>
                </div>
              </label>
              <label className={`auth-role-option ${selectedRole === 'MEMBER' ? 'selected' : ''}`}>
                <input type="radio" value="MEMBER" {...register('role')} />
                <User size={18} />
                <div>
                  <span className="auth-role-title">Member</span>
                  <span className="auth-role-desc">Join projects & manage tasks</span>
                </div>
              </label>
            </div>
